package br.com.mygym.uitoolkit.button
...
class LoaderButton @JvmOverloads constructor(
    ...
    private fun applySecondaryStyle() {
        button.background = ContextCompat.getDrawable(context, R.drawable.background_button_secondary)
        button.setTextColor(ContextCompat.getColor(context, R.color.main_color))
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            progressBar.indeterminateDrawable.colorFilter = BlendModeColorFilter(ContextCompat.getColor(context, R.color.main_color), BlendMode.SRC_ATOP)
        } else {
            progressBar.indeterminateDrawable.setColorFilter(ContextCompat.getColor(context, R.color.main_color), PorterDuff.Mode.SRC_ATOP)
        }
    }
